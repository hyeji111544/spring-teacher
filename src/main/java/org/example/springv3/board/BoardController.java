...
public class BoardController {
    ...
    public String list(@RequestParam(name = "title", required = false) String title,
        ...
        BoardResponse.PageDTO pageDTO =  boardService.게시글목록보기v2(title, page);
    ...
    public @ResponseBody Resp testV2() {
        // 두명 처리할려면 arryList 에 넣어야함 (중괗호 아니고 대괄호로 들어옴)
        User u1 = new User();
        u1.setId(1);
        u1.setUsername("ssar");
        u1.setPassword("1234");
    ...
    public @ResponseBody Resp testV3() {
       // throw new Exception404("유저를 찾을 수 없습니다"); 이건 에러 발생 시키지만 html 리턴이라 틀림
        return Resp.fail(404, "유저를 찾을 수 없습니다."); // json 데이터로 응답해야한다.
    }
    @GetMapping("/test/v4")
